...
type SpawnedItem = {
  ...
  // Groups for 3D neon models from grid-items.ts.
  // `kind` lets animateItems/collision skip irrelevant branches cleanly.
  object3D: Sprite | Mesh | Object3D;
  kind: 'sprite' | 'wall' | 'model';
  role: ItemRole;
...
export class PortalSystem extends createSystem({}) {
  ...
  private spawnItem(
    ...
    if (neonModel) {
      ...
      this.spawnedEntities.set(key, {
        entity, object3D: neonModel, kind: 'model', role, type, baseSize,
        origin: [x, y, z],
        hp: role === 'enemy' ? enemyStats(type).hp : role === 'bird' ? BIRD_HP : 0,
        nextDamageableAt: 0,
        heading: 0, radius: 0, omega: 0, phase: 0,
        birdState: 'flying', hitFlashUntil: 0, extra,
        knockbackUntil: 0, knockbackVx: 0, knockbackVz: 0,
      });
      return;
    }

    // Fallback: emoji sprite with role-driven halo/aura.
  ...
  private despawnItem(key: string) {
    ...
    if (record.kind === 'sprite') {
      ...
      // 3D neon model from grid-items.ts — dispose all child geometries + materials
      record.object3D.traverse((child: any) => {
